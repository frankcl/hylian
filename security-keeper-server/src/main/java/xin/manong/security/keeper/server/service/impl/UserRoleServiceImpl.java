...
import xin.manong.security.keeper.server.dao.mapper.UserRoleMapper;
import xin.manong.security.keeper.server.service.UserRoleService;
...
public class UserRoleServiceImpl implements UserRoleService {
    ...
    public boolean add(UserRole userRole) {
        ...
        if (userRoleMapper.selectCount(query) > 0) {
            ...
            throw new RuntimeException("用户角色关系已存在");
        }
    ...
    public boolean delete(Long id) {
        if (id == null) {
            ...
            throw new RuntimeException("用户角色关系ID为空");
        }
