...
public class TicketServiceImplSuite {
    ...
    public void testTicketCache() {
        ...
        Set<String> tokens = ticketService.getTokens(profile.id);
        ...
        tokens = ticketService.getTokens(profile.id);
        Assert.assertTrue(tokens != null && tokens.size() == 1);
        Assert.assertTrue(tokens.contains(DigestUtils.md5Hex("token2")));
        ticketService.removeTokens(profile.id);
        Assert.assertTrue(ticketService.getTokens(profile.id).isEmpty());
    }
}
